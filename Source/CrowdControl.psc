...
Function ProcessCommand(int id, String command, String viewer, int type)
    ...
    hour.SetValue(0)
    Respond(id, 0, viewer + " changed the time")
    ...
    bool view = Game.GetPlayer().GetAnimationVariableInt("IsFirstPerson")

    if view
        Respond(id, 1, "Already in first person")
    else
        Respond(id, 0, viewer + " forced first person")
    ...
    if view
        Respond(id, 0, viewer + " forced third person")
        Game.ForceThirdPerson()
    else
        Respond(id, 1, "Already in third person")
    endif
    elseif command == "blood"
    Game.TriggerScreenBlood(25)
    Respond(id, 4, viewer + " splashed blood on the screen", 10000)
    elseif command == "jail"
    PoliceFaction.SendPlayerToJail()
    Respond(id, 0, viewer + " sent you to jail")
    ...
    elseif command == "random_fast_travel"
        ...
        elseif StringUtil.Find(command, "spawn_") == 0
            ...
            if !canSpawn
                ...
                if spawn == None
                    ...
                    ObjectReference ref = self.PlaceAtMe(spawn)
        ...
        elseif StringUtil.Find(command, "weather_") == 0
            string spawnName = StringUtil.Substring(command, 8)
            ...
            Respond(id, 1, "Weather Already Active")
            else
                    Weather spawn = GetWeatherRef(spawnName)
                    if spawn == None
                            Respond(id, 2, viewer + " tried starting invalid weather \"" + spawnName + "\"")
                        else
                            spawn.ForceActive(true)
                weatherOverride = true
                    Respond(id, 4, viewer + " made the weather " + spawnName, 30000)
                        endif
                    endif
                else
                weatherOverride = false
                Weather.ReleaseOverride()
                    Respond(id, 0, "Weather ended")
                endif



        elseif StringUtil.Find(command, "spawnb_") == 0
            string spawnName = StringUtil.Substring(command, 7)

            if !canSpawn
                Respond(id, 3)
            else
                ActorBase spawn = GetSpawnRefB(spawnName)
                if spawn == None
                    Respond(id, 2, viewer + " tried spawning invalid actor \"" + spawnName + "\"")
                else
                    ObjectReference ref = self.PlaceAtMe(spawn)
        ...
        elseif StringUtil.Find(command, "race_") == 0
            string raceName = StringUtil.Substring(command, 5)

        Race PlayerRace = Game.GetPlayer().GetRace()
                Race raceref = GetRaceRef(raceName)
                if raceref == None
                    Respond(id, 2, viewer + " tried setting invalid race \"" + raceName + "\"")
        elseif PlayerRace == raceref                
                    Respond(id, 1, "Player is already race \"" + raceName + "\"")
        else
                    Game.GetPlayer().SetRace(raceref)
                    Respond(id, 0, viewer + " changed your race to " + raceName)
                endif
